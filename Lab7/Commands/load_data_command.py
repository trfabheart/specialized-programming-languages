...
class LoadDataCommand(Command):
    ...
    def execute(self):
        ...
        if posts_data:
            ...
            self.user_interface.display_message("Posts upload successfully!", color=Fore.GREEN)

        albums_data = APIClient.get_data("albums")
        if albums_data:
            self.albums.extend(albums_data)
            self.history.log_request("GET /albums", albums_data)
            self.user_interface.display_message("Albums upload successfully!", color=Fore.GREEN)
